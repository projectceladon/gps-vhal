 ...
 */
// #define LOG_NDEBUG 0
#define LOG_TAG "VirtualCamera_HotplugThread"
#include <log/log.h>
...
#define SubscriberInfo VirtualCameraHotplugThread::SubscriberInfo

namespace android {

VirtualCameraHotplugThread::VirtualCameraHotplugThread(const int *cameraIdArray, size_t size)
...
VirtualCameraHotplugThread::~VirtualCameraHotplugThread() {}

status_t VirtualCameraHotplugThread::requestExitAndWait() {
    ALOGE("%s: Not implemented. Use requestExit + join instead", __FUNCTION__);
    return INVALID_OPERATION;
}

void VirtualCameraHotplugThread::requestExit() {
    ...
    if (rmWatchFailed) {  // unlikely
        ...
        if (close(mInotifyFd) == -1) {
            ALOGE("%s: close failure error: '%s' (%d)", __FUNCTION__, strerror(errno), errno);
        }
    }
...
status_t VirtualCameraHotplugThread::readyToRun() {
    Mutex::Autolock al(mMutex);
    ...
    do {
        ...
        if (mInotifyFd == -1) {
            ALOGE("%s: inotify_init failure error: '%s' (%d)", __FUNCTION__, strerror(errno),
                  errno);
            mRunning = false;
            break;
        }
...
bool VirtualCameraHotplugThread::threadLoop() {
        ...
        while (i < length) {
            ...
            if (event->mask & IN_IGNORED) {
                ...
                if (cameraId < 0) {
                    ...
                    int halStatus =
                        newStatus ? CAMERA_DEVICE_STATUS_PRESENT : CAMERA_DEVICE_STATUS_NOT_PRESENT;
                    gVirtualCameraFactory.onStatusChanged(cameraId, halStatus);
                }
            } else {
...
String8 VirtualCameraHotplugThread::getFilePath(int cameraId) const {
    return String8::format(FAKE_HOTPLUG_FILE ".%d", cameraId);
}

bool VirtualCameraHotplugThread::createFileIfNotExists(int cameraId) const {
    String8 filePath = getFilePath(cameraId);
    // make sure this file exists and we have access to it
    int fd = TEMP_FAILURE_RETRY(open(filePath.string(), O_WRONLY | O_CREAT | O_TRUNC,
                                     /* mode = ug+rwx */ S_IRWXU | S_IRWXG));
    if (fd == -1) {
        ALOGE("%s: Could not create file '%s', error: '%s' (%d)", __FUNCTION__, filePath.string(),
              strerror(errno), errno);
        return false;
    }
    ...
    if (TEMP_FAILURE_RETRY(write(fd, "1\n", /*count*/ 2)) == -1) {
        ALOGE("%s: Could not write '1' to file '%s', error: '%s' (%d)", __FUNCTION__,
...
int VirtualCameraHotplugThread::getCameraId(const String8 &filePath) const {
    Vector<int>::const_iterator it, end;
    it = mSubscribedCameraIds.begin();
    end = mSubscribedCameraIds.end();
    for (; it != end; ++it) {
...
int VirtualCameraHotplugThread::getCameraId(int wd) const {
...
SubscriberInfo *VirtualCameraHotplugThread::getSubscriberInfo(int cameraId) {
    for (size_t i = 0; i < mSubscribers.size(); ++i) {
...
bool VirtualCameraHotplugThread::addWatch(int cameraId) {
    String8 camPath = getFilePath(cameraId);
    int wd = inotify_add_watch(mInotifyFd, camPath.string(), IN_CLOSE_WRITE);

    if (wd == -1) {
        ALOGE("%s: Could not add watch for '%s', error: '%s' (%d)", __FUNCTION__, camPath.string(),
              strerror(errno), errno);
...
bool VirtualCameraHotplugThread::removeWatch(int cameraId) {
    SubscriberInfo *si = getSubscriberInfo(cameraId);

    if (!si) return false;

    if (inotify_rm_watch(mInotifyFd, si->WatchID) == -1) {
        ALOGE("%s: Could not remove watch for camID '%d', error: '%s' (%d)", __FUNCTION__, cameraId,
              strerror(errno), errno);
...
int VirtualCameraHotplugThread::readFile(const String8 &filePath) const {
    int fd = TEMP_FAILURE_RETRY(open(filePath.string(), O_RDONLY, /*mode*/ 0));
    if (fd == -1) {
        ALOGE("%s: Could not open file '%s', error: '%s' (%d)", __FUNCTION__, filePath.string(),
              strerror(errno), errno);
        return -1;
    }
    ...
    ALOGV("%s: Read file '%s', length='%d', buffer='%c'", __FUNCTION__, filePath.string(), length,
          buffer[0]);
