...
VirtualCameraFactory::~VirtualCameraFactory() {
    ...
    if (mRemoteClient.mCameraSocketFD > 0) {
        ALOGV("%s Close camera client fd(mRemoteClient.mCameraSocketFD=%d)", __FUNCTION__,
              mRemoteClient.mCameraSocketFD);
        ...
        switch (curCameraId) {
            case 0:
                ...
                if (mVirtualCameraNum >= 3 && mVirtualCameras[2] != nullptr) {
                    cameraId = 2;
                    ALOGV("%s map curCameraId(%d) to cameraId(%d)", __FUNCTION__, curCameraId,
                          cameraId);
                }
                break;
            case 1:
...
bool VirtualCameraFactory::getTokenValue(const char *token, const std::string &s, char **value) {
...
void VirtualCameraFactory::findRemoteCameras(std::vector<RemoteCameraInfo> *remoteCameras) {
     ...
     */
    if (res != NO_ERROR || cameraList == nullptr || *cameraList == '\0' || *cameraList == '\n') {
...
void VirtualCameraFactory::createRemoteCameras(const std::vector<RemoteCameraInfo> &remoteCameras) {
    /*
    ...
    for (const auto &cameraInfo : remoteCameras) {
        ...
        switch (halVersion) {
            case 1:
                ...
                remoteCamOne =
                    new VirtualRemoteCamera(mVirtualCameraNum, &HAL_MODULE_INFO_SYM.common);
                if (remoteCamOne == nullptr) {
                    ALOGE("%s: Unable to instantiate VirtualRemoteCamera", __FUNCTION__);
                } else {
                    /*
                     ...
                     */
                    res = remoteCamOne->Initialize(cameraInfo.name, cameraInfo.frameDims,
                                                   cameraInfo.dir);
                }
            ...
            case 2:
                ALOGE(
                    "%s: REMOTE support for camera hal version %d is not "
                    "implemented",
            ...
            case 3:
                ...
                remoteCamThree =
                    new VirtualRemoteCamera3(mVirtualCameraNum, &HAL_MODULE_INFO_SYM.common);
                if (remoteCamThree == nullptr) {
                    ALOGE("%s: Unable to instantiate VirtualRemoteCamera3", __FUNCTION__);
                } else {
                    res = remoteCamThree->Initialize(cameraInfo.name, cameraInfo.frameDims,
                                                     cameraInfo.dir);
                }
                remoteCam = remoteCamThree;
                break;
            default:
                ALOGE("%s: Unknown camera hal version requested: %d", __FUNCTION__, halVersion);
        }
    ...
    switch (halVersion) {
        case 1:
            mVirtualCameras[mVirtualCameraNum] =
                new VirtualFakeCamera(mVirtualCameraNum, backCamera, &HAL_MODULE_INFO_SYM.common);
            break;
        case 2:
            mVirtualCameras[mVirtualCameraNum] =
