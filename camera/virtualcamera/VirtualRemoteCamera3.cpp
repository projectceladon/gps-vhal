...
/*
 * Contains implementation of a class VirtualRemoteCamera3 that encapsulates
...
const int32_t VirtualRemoteCamera3::kAvailableFormats[] = {
    HAL_PIXEL_FORMAT_BLOB, HAL_PIXEL_FORMAT_RGBA_8888, HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED,
...
const nsecs_t VirtualRemoteCamera3::kNormalExposureTime = 10 * MSEC;
const nsecs_t VirtualRemoteCamera3::kFacePriorityExposureTime = 30 * MSEC;
const int VirtualRemoteCamera3::kNormalSensitivity = 100;
const int VirtualRemoteCamera3::kFacePrioritySensitivity = 400;
// CTS requires 8 frames timeout in waitForAeStable
const float VirtualRemoteCamera3::kExposureTrackRate = 0.2;
const int VirtualRemoteCamera3::kPrecaptureMinFrames = 10;
const int VirtualRemoteCamera3::kStableAeMaxFrames = 100;
const float VirtualRemoteCamera3::kExposureWanderMin = -2;
const float VirtualRemoteCamera3::kExposureWanderMax = 1;

/*****************************************************************************
 * Constructor/Destructor
 ****************************************************************************/

VirtualRemoteCamera3::VirtualRemoteCamera3(int cameraId, struct hw_module_t *module)
...
VirtualRemoteCamera3::~VirtualRemoteCamera3() {
    for (size_t i = 0; i < CAMERA3_TEMPLATE_COUNT; ++i) {
...
void VirtualRemoteCamera3::parseResolutions(const char *frameDims) {
    ...
    if (frameDimsLength == kMaxFrameDimsLength) {
        ...
        ALOGE("%s: Frame dimensions string was NULL or zero-length", __FUNCTION__);
        return;
    }
    ...
    for (auto res = mResolutions.begin(); res != mResolutions.end();) {
        if (res->first > (int32_t)mSensorWidth || res->second > (int32_t)mSensorHeight) {
    ...
    if (mResolutions.empty()) {
        ALOGE("%s: Remote camera has no valid resolutions", __FUNCTION__);
    }
}

status_t VirtualRemoteCamera3::Initialize(const char *deviceName, const char *frameDims,
...
status_t VirtualRemoteCamera3::connectCamera(hw_device_t **device) {
     ...
     * Initialize sensor.
     */
    mSensor = new RemoteSensor(mDeviceName, mSensorWidth, mSensorHeight);
    if (mSensor != nullptr && mCameraSocketFD > 0) {
        ALOGV("%s: RemoteSensor set camera fd(%d)", __FUNCTION__, mCameraSocketFD);
        mSensor->setCameraFD(mCameraSocketFD);
    } else {
        ALOGV("%s: mSensor is nullptr of mCameraSocketFD(%d) less than 0", __FUNCTION__,
              mCameraSocketFD);
    }
    mSensor->setRemoteSensorListener(this);
    res = mSensor->startUp();
    if (res != NO_ERROR) {
        return res;
    }
...
status_t VirtualRemoteCamera3::closeCamera() {
    status_t res;
    {
        Mutex::Autolock l(mLock);
...
status_t VirtualRemoteCamera3::cleanCameraFD(int socketFd) {
    if (mSensor != nullptr) {
        mSensor->cleanCameraFD(socketFd);
    }
    mCameraSocketFD = -1;
...
status_t VirtualRemoteCamera3::configureStreams(camera3_stream_configuration *streamList) {
    Mutex::Autolock l(mLock);
    ...
    for (size_t i = 0; i < streamList->num_streams; ++i) {
        ...
        ALOGV("%s: Stream %p (id %zu), type %d, usage 0x%x, format 0x%x", __FUNCTION__, newStream,
              i, newStream->stream_type, newStream->usage, newStream->format);
        ...
        size_t numFormats = sizeof(kAvailableFormats) / sizeof(kAvailableFormats[0]);
...
status_t VirtualRemoteCamera3::registerStreamBuffers(const camera3_stream_buffer_set *bufferSet) {
    Mutex::Autolock l(mLock);
    ALOGE("%s: Should not be invoked on HAL versions >= 3.2!", __FUNCTION__);
    return NO_INIT;
}

const camera_metadata_t *VirtualRemoteCamera3::constructDefaultRequestSettings(int type) {
    ALOGE(" %s", __FUNCTION__);
...
status_t VirtualRemoteCamera3::processCaptureRequest(camera3_capture_request *request) {
    Mutex::Autolock l(mLock);
    status_t res;
    ...
    if (request->input_buffer != nullptr && request->input_buffer->stream != mInputStream) {
    ...
    exposureTime = (entry.count > 0) ? entry.data.i64[0] : RemoteSensor::kExposureTimeRange[0];
    ...
    frameDuration = (entry.count > 0) ? entry.data.i64[0] : RemoteSensor::kFrameDurationRange[0];

    if (exposureTime > frameDuration) {
        frameDuration = exposureTime + RemoteSensor::kMinVerticalBlank;
        settings.update(ANDROID_SENSOR_FRAME_DURATION, &frameDuration, 1);
    }

    static const int32_t sensitivity = RemoteSensor::kSensitivityRange[0];
            ...
            if (srcBuf.stream->format == HAL_PIXEL_FORMAT_YCbCr_420_888) {
                if (destBuf.format == HAL_PIXEL_FORMAT_YCbCr_420_888) {
                    ...
                    res = GrallocModule::getInstance().lock_ycbcr(
                        *(destBuf.buffer), GRALLOC_USAGE_HW_CAMERA_WRITE, 0, 0, destBuf.width,
                ...
                res = GrallocModule::getInstance().lock(
                    *(destBuf.buffer), GRALLOC_USAGE_HW_CAMERA_WRITE, 0, 0, destBuf.width,
                    destBuf.height, (void **)&(destBuf.img));
            }
            if (res != OK) {
    ...
    if (needJpeg) {
        ...
        if (res != OK) {
            ALOGE(
                "%s: Error managing JPEG compressor resources, can't "
                "reserve it!",
                __FUNCTION__);
            return NO_INIT;
        }
...
status_t VirtualRemoteCamera3::flush() {
...
bool VirtualRemoteCamera3::hasCapability(AvailableCapabilities cap) {
    ssize_t idx = mCapabilities.indexOf(cap);
    return idx >= 0;
}

status_t VirtualRemoteCamera3::constructStaticInfo() {
    CameraMetadata info;
...
#define ADD_STATIC_ENTRY(name, varptr, count) \
    ...
    for (const auto &res : mResolutions) {
        ...
        std::vector<int32_t> currentResStreamConfigurations = {
            ...
            height,
            ANDROID_SCALER_AVAILABLE_STREAM_CONFIGURATIONS_OUTPUT};
        std::vector<int32_t> currentResMinFrameDurations = {HAL_PIXEL_FORMAT_BLOB,
                                                            width,
                                                            height,
                                                            RemoteSensor::kFrameDurationRange[0],
    ...
    if (hasCapability(BACKWARD_COMPATIBLE)) {
        static const int32_t max3aRegions[] = {/* AE */ 1,
                                               /* AWB */ 0,
                                               /* AF */ 1};
    ...
    static const int32_t availableTargetFpsRanges[] = {5, 30, 15, 30, 15, 15, 30, 30};
    ...
    static const uint8_t availableAfModesBack[] = {ANDROID_CONTROL_AF_MODE_OFF};
    ...
    static const uint8_t supportedHardwareLevel = ANDROID_INFO_SUPPORTED_HARDWARE_LEVEL_LIMITED;
    ADD_STATIC_ENTRY(ANDROID_INFO_SUPPORTED_HARDWARE_LEVEL, &supportedHardwareLevel, /* count */ 1);
    ...
    ADD_STATIC_ENTRY(ANDROID_REQUEST_PARTIAL_RESULT_COUNT, &partialResultCount, /* count */ 1);
...
status_t VirtualRemoteCamera3::process3A(CameraMetadata &settings) {
    /**
...
status_t VirtualRemoteCamera3::doFakeAE(CameraMetadata &settings) {
    camera_metadata_entry e;
...
status_t VirtualRemoteCamera3::doFakeAF(CameraMetadata &settings) {
    camera_metadata_entry e;
    ...
    switch (afMode) {
        ...
        case ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE:
            // Always report INACTIVE for Remote Camera
            mAfState = ANDROID_CONTROL_AF_STATE_INACTIVE;
            break;
        default:
...
status_t VirtualRemoteCamera3::doFakeAWB(CameraMetadata &settings) {
    camera_metadata_entry e;
    ...
    switch (awbMode) {
        ...
        case ANDROID_CONTROL_AWB_MODE_SHADE:
            // Always magically right for Remote Camera
...
void VirtualRemoteCamera3::update3A(CameraMetadata &settings) {
...
void VirtualRemoteCamera3::signalReadoutIdle() {
    Mutex::Autolock l(mLock);
    /*
...
void VirtualRemoteCamera3::onRemoteSensorEvent(uint32_t frameNumber, Event e, nsecs_t timestamp) {
    switch (e) {
...
VirtualRemoteCamera3::ReadoutThread::ReadoutThread(VirtualRemoteCamera3 *parent)
    : mParent(parent), mJpegWaiting(false) {
    ALOGV("%s: Creating readout thread", __FUNCTION__);
}

VirtualRemoteCamera3::ReadoutThread::~ReadoutThread() {
...
void VirtualRemoteCamera3::ReadoutThread::queueCaptureRequest(const Request &r) {
    Mutex::Autolock l(mLock);
...
bool VirtualRemoteCamera3::ReadoutThread::isIdle() {
    Mutex::Autolock l(mLock);
    return mInFlightQueue.empty() && !mThreadActive;
}

status_t VirtualRemoteCamera3::ReadoutThread::waitForReadout() {
    status_t res;
    Mutex::Autolock l(mLock);
    int loopCount = 0;
...
bool VirtualRemoteCamera3::ReadoutThread::threadLoop() {
    status_t res;
...
void VirtualRemoteCamera3::ReadoutThread::onJpegDone(const StreamBuffer &jpegBuffer, bool success) {
    Mutex::Autolock jl(mJpegLock);
...
void VirtualRemoteCamera3::ReadoutThread::onJpegInputDone(const StreamBuffer &inputBuffer) {
    /*
