 ...
 public:
  ...
  int lock(buffer_handle_t handle, int usage, int left, int top, int width,
    ...
    switch (m_major_version) {
      case 0: {
        return mModule->lock(mModule, handle, usage, left, top, width, height,
                             vaddr);
      }
      case 1:
#ifdef USE_GRALLOC1
      {
        ...
        return m_gralloc1_lock(m_gralloc1_device, handle, usage, usage,
                               &outRect, vaddr, -1);
      }
#endif
      default: {
  ...
  int lock_ycbcr(buffer_handle_t handle, int usage, int left, int top,
                 int width, int height, struct android_ycbcr *ycbcr) {
    switch (m_major_version) {
      case 0: {
        return mModule->lock_ycbcr(mModule, handle, usage, left, top, width,
                                   height, ycbcr);
      }
      case 1:
#ifdef USE_GRALLOC1
  ...
  GrallocModule() {
    ...
    switch (m_major_version) {
      ...
      case 1:
        ...
        m_gralloc1_lockflex =
            (GRALLOC1_PFN_LOCK_FLEX)m_gralloc1_device->getFunction(
