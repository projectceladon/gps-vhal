...
#ifndef HW_EMULATOR_CAMERA_VIRTUALD_CAMERA_DEVICE_H
#define HW_EMULATOR_CAMERA_VIRTUALD_CAMERA_DEVICE_H

/*
 * Contains declaration of an abstract class VirtualCameraDevice that defines
...
/* Encapsulates an abstract class VirtualCameraDevice that defines
 * functionality expected from an virtual physical camera device:
 *  - Obtaining and setting camera device parameters
 *  - Capturing frames
 *  - Streaming video
...
public:
    /* Constructs VirtualCameraDevice instance.
     * Param:
     *  camera_hal - Virtual camera that implements the camera HAL API, and
     *      manages (contains) this object.
     */
    explicit VirtualCameraDevice(VirtualCamera *camera_hal);

    /* Destructs VirtualCameraDevice instance. */
    virtual ~VirtualCameraDevice();

    /***************************************************************************
     * Virtual camera device abstract interface
     **************************************************************************/
...
public:
     ...
     * the delivery of the captured frames to the virtual camera. Call
     ...
     * this method doesn't stop delivering frames to the virtual camera. Always
...
public:
    /* Initializes VirtualCameraDevice instance.
     ...
     */
    void initializeWhiteBalanceModes(const char *mode, const float r_scale, const float b_scale);
     ...
     * the camera device, and will deliver the pulled frames back to the virtual
     ...
     *      virtual camera. If this parameter is 'false', frames will keep
     ...
     *  NO_ERROR on success, or an appropriate error status.
     */
    virtual status_t getCurrentFrame(void *buffer, uint32_t pixelFormat, int64_t *timestamp);
    ...
    class FrameLock {
    public:
        FrameLock(VirtualCameraDevice &cameraDevice);
        ~FrameLock();
     ...
     * Since virtual camera device gets its data from the actual device, it gets
    ...
    inline bool isInitialized() const { return mState != ECDS_CONSTRUCTED; }
     ...
     */
    bool requestRestart(int width, int height, uint32_t pixelFormat, bool takingPicture,
                        bool oneBurst);
...
protected:
    ...
    inline uint8_t changeExposure(const uint8_t &inputY) const {
        return static_cast<uint8_t>(clamp(static_cast<float>(inputY) * mExposureCompensation));
    }
     ...
     */
    status_t getCurrentFrameImpl(const uint8_t *source, uint8_t *dest, uint32_t pixelFormat) const;

    /****************************************************************************
     * Worker thread management.
     * Typicaly when virtual camera device starts capturing frames from the
     ...
     * for all virtual camera devices.
     ***************************************************************************/
...
protected:
    ...
    virtual void *getPrimaryBuffer() { return mFrameBuffers[0].data(); }

    /* Get the seconary buffer to use when constructing the FrameProducer. */
    virtual void *getSecondaryBuffer() { return mFrameBuffers[1].data(); }
    ...
    public:
        ...
        CameraThread(VirtualCameraDevice *cameraDevice, ProduceFrameFunc producer,
                     void *producerOpaque);
        ...
        void requestRestart(int width, int height, uint32_t pixelFormat, bool takingPicture,
                            bool oneBurst);
    ...
    private:
        ...
        public:
            FrameProducer(VirtualCameraDevice *cameraDevice, ProduceFrameFunc producer,
                          void *opaque, void *primaryBuffer, void *secondaryBuffer);
...
protected:
    ...
    /* Virtual camera object containing this instance. */
    VirtualCamera *mCameraHAL;
...
private:
    ...
    static bool staticProduceFrame(void *opaque, void *buffer, int64_t *timestamp) {
        auto cameraDevice = reinterpret_cast<VirtualCameraDevice *>(opaque);
...
#endif /* HW_EMULATOR_CAMERA_VIRTUALD_CAMERA_DEVICE_H */
