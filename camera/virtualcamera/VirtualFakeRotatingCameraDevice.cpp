...
/*
 * Contains implementation of a class VirtualFakeRotatingCameraDevice that encapsulates
 * fake camera device.
 */
...
void VirtualFakeRotatingCameraDevice::render(int width, int height) {
    update_scene((float)width, (float)height);
    create_texture_dotx(1280, 720);

    int w = 992 / 2;
    int h = 1280 / 2;
    const GLfloat verticesfloat[] = {-w, -h, 0, w, -h, 0, w, h, 0, -w, h, 0};

    const GLfloat texCoordsfloat[] = {0, 0, 1.0f, 0, 1.0f, 1.0f, 0, 1.0f};
...
void VirtualFakeRotatingCameraDevice::create_texture_dotx(int width, int height) {
...
static void gluLookAt(float eyeX, float eyeY, float eyeZ, float centerX, float centerY,
...
void VirtualFakeRotatingCameraDevice::update_scene(float width, float height) {
...
void VirtualFakeRotatingCameraDevice::free_gl_surface(void) {
...
void VirtualFakeRotatingCameraDevice::init_sensor() {
...
void VirtualFakeRotatingCameraDevice::read_sensor() {
    if (mSensorPipe < 0) return;
    char get[] = "get";
    ...
    ALOGD("accel: %g %g %g; magnetic %g %g %g orientation %g %g %g",
          ...
          mSensorValues[SENSOR_VALUE_ACCEL_Z], mSensorValues[SENSOR_VALUE_MAGNETIC_X],
          mSensorValues[SENSOR_VALUE_MAGNETIC_Y], mSensorValues[SENSOR_VALUE_MAGNETIC_Y],
          mSensorValues[SENSOR_VALUE_ROTATION_X], mSensorValues[SENSOR_VALUE_ROTATION_Y],
          mSensorValues[SENSOR_VALUE_ROTATION_Z]);
}

void VirtualFakeRotatingCameraDevice::read_rotation_vector(double *yaw, double *pitch,
                                                           double *roll) {
    read_sensor();
...
void VirtualFakeRotatingCameraDevice::get_yawing(float *x, float *y, float *z) {
...
void VirtualFakeRotatingCameraDevice::get_eye_x_y_z(float *x, float *y, float *z) {
    const float R = 3500;
...
int VirtualFakeRotatingCameraDevice::init_gl_surface(int width, int height) {
    EGLint numConfigs = 1;
...
VirtualFakeRotatingCameraDevice::VirtualFakeRotatingCameraDevice(VirtualFakeCamera *camera_hal)
    : VirtualCameraDevice(camera_hal), mOpenglReady(false) {}

VirtualFakeRotatingCameraDevice::~VirtualFakeRotatingCameraDevice() {}

/****************************************************************************
 * Virtual camera device abstract interface implementation.
 ***************************************************************************/

status_t VirtualFakeRotatingCameraDevice::connectDevice() {
...
status_t VirtualFakeRotatingCameraDevice::disconnectDevice() {
    ALOGV("%s", __FUNCTION__);
...
status_t VirtualFakeRotatingCameraDevice::startDevice(int width, int height, uint32_t pix_fmt) {
    ...
    const status_t res = VirtualCameraDevice::commonStartDevice(width, height, pix_fmt);
...
status_t VirtualFakeRotatingCameraDevice::stopDevice() {
    ALOGV("%s", __FUNCTION__);
...
bool VirtualFakeRotatingCameraDevice::produceFrame(void *buffer, int64_t *timestamp) {
...
void VirtualFakeRotatingCameraDevice::fillBuffer(void *buffer) {
