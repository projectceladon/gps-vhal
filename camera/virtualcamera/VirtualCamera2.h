...
#ifndef HW_EMULATOR_CAMERA_VIRTUALD_CAMERA2_H
#define HW_EMULATOR_CAMERA_VIRTUALD_CAMERA2_H
...
/* Encapsulates functionality common to all version 2.0 virtual camera devices
 *
 * Note that VirtualCameraFactory instantiates object of this class just once,
 * when VirtualCameraFactory instance gets constructed. Connection to /
 ...
 */
class VirtualCamera2 : public camera2_device, public VirtualBaseCamera {
public:
     ...
     */
    VirtualCamera2(int cameraId, struct hw_module_t *module);

    /* Destructs VirtualCamera2 instance. */
    virtual ~VirtualCamera2();
...
protected:
     ...
     * dispatch the call into an instance of VirtualCamera2 class defined in
...
private:
    ...
    static int construct_default_request(const camera2_device_t *, int request_template,
                                         camera_metadata_t **request);

    /** Stream management */
    static int allocate_stream(const camera2_device_t *, uint32_t width, uint32_t height,
                               int format, const camera2_stream_ops_t *stream_ops,
                               uint32_t *stream_id, uint32_t *format_actual, uint32_t *usage,
                               uint32_t *max_buffers);

    static int register_stream_buffers(const camera2_device_t *, uint32_t stream_id,
                                       int num_buffers, buffer_handle_t *buffers);

    static int release_stream(const camera2_device_t *, uint32_t stream_id);

    static int allocate_reprocess_stream(const camera2_device_t *, uint32_t width, uint32_t height,
                                         uint32_t format,
                                         const camera2_stream_in_ops_t *reprocess_stream_ops,
                                         uint32_t *stream_id, uint32_t *consumer_usage,
                                         uint32_t *max_buffers);
    ...
    static int release_reprocess_stream(const camera2_device_t *, uint32_t stream_id);

    /** 3A triggers*/
    static int trigger_action(const camera2_device_t *, uint32_t trigger_id, int ext1, int ext2);

    /** Notifications to application */
    static int set_notify_callback(const camera2_device_t *, camera2_notify_callback notify_cb,
                                   void *user);

    /** Vendor metadata registration */
    static int get_metadata_vendor_tag_ops(const camera2_device_t *, vendor_tag_query_ops_t **ops);
    // for get_metadata_vendor_tag_ops
    static const char *get_camera_vendor_section_name(const vendor_tag_query_ops_t *, uint32_t tag);
    static const char *get_camera_vendor_tag_name(const vendor_tag_query_ops_t *, uint32_t tag);
    static int get_camera_vendor_tag_type(const vendor_tag_query_ops_t *, uint32_t tag);
...
protected:
    ...
    struct TagOps : public vendor_tag_query_ops {
        VirtualCamera2 *parent;
    };
    TagOps mVendorTagOps;

    void sendNotification(int32_t msgType, int32_t ext1, int32_t ext2, int32_t ext3);
...
#endif /* HW_EMULATOR_CAMERA_VIRTUALD_CAMERA2_H */
