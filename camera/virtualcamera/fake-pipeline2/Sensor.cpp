...
bool Sensor::threadLoop() {
  ...
  /**
   * Stage 2: Read out latest captured image
   */
...
void Sensor::captureRGBA(uint8_t *img, uint32_t gain, uint32_t width,
  ...
  // I420 to RGBA coversion using libYUV API
  if (int ret = libyuv::I420ToABGR(pTempY, 640, pTempU, 320, pTempV, 320, img,
                                   640 * 4, 640, 480)) {
  }

  ALOGVV("[Kausha] %s Done with converion into img[%p]", __FUNCTION__, img);

// Dump RGBA frame after convertion from I420 frame format
#if 0
      if(j<300){
	      //Dump 10 sec frames
	      ALOGE("[Kausha] %s Dump RGBA[%d]",__FUNCTION__, j);
	      DUMP_RGBA(j,img,1228800);
      }
      j++;
#endif
// Below Conversion logic (I420 to RGBA) takes <1ms for non-SG1 environment but
// takes ~195ms for SG1 env
#if 0
	uint32_t w, h;
...
void Sensor::saveNV21(uint8_t *img, uint32_t size) {
  ...
  if (NULL == f) {
    ALOGVV("%s:%d Fail to open /data/local/tmp/savenv21.nv21.", __func__,
           __LINE__);
    return;
  }
