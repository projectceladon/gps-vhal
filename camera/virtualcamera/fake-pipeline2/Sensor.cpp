...
#define LOG_TAG "VirtualCamera2_Sensor"
...
namespace android
{
    ...
    Sensor::Sensor(uint32_t width, uint32_t height) : Thread(false),
    ...
    status_t Sensor::startUp()
    {
        ...
        int res;
        mCapturedBuffers = NULL;
        res = run("VirtualFakeCamera2::Sensor",
    ...
    void Sensor::captureRaw(uint8_t *img, uint32_t gain, uint32_t stride)
    {
        ...
        for (unsigned int y = 0; y < mResolution[1]; y++)
        {
            ...
            for (unsigned int x = 0; x < mResolution[0]; x++)
            {
                ...
                electronCount = (electronCount < kSaturationElectrons) ? electronCount : kSaturationElectrons;
