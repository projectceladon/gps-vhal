...
/*
 * Contains implementation of a class VirtualFakeCameraDevice that encapsulates
 * fake camera device.
 */
...
VirtualFakeCameraDevice::VirtualFakeCameraDevice(VirtualFakeCamera *camera_hal)
...
VirtualFakeCameraDevice::~VirtualFakeCameraDevice() {}

/****************************************************************************
 * Virtual camera device abstract interface implementation.
 ***************************************************************************/

status_t VirtualFakeCameraDevice::connectDevice() {
...
status_t VirtualFakeCameraDevice::disconnectDevice() {
    ALOGV("%s", __FUNCTION__);
...
status_t VirtualFakeCameraDevice::startDevice(int width, int height, uint32_t pix_fmt) {
    ALOGV("%s", __FUNCTION__);
...
status_t VirtualFakeCameraDevice::stopDevice() {
    ALOGV("%s", __FUNCTION__);
...
bool VirtualFakeCameraDevice::produceFrame(void *buffer, int64_t *timestamp) {
#if EFCD_ROTATE_FRAME
...
void VirtualFakeCameraDevice::drawCheckerboard(void *buffer) {
    ...
    if (mCheckX >= 1.0) black = false;
    if (mCheckY >= 1.0) black = !black;
...
void VirtualFakeCameraDevice::drawSquare(void *buffer, int x, int y, int size,
...
void VirtualFakeCameraDevice::drawSolid(void *buffer, YUVPixel *color) {
    YUVPixel adjustedColor = *color;
    changeWhiteBalance(adjustedColor.Y, adjustedColor.U, adjustedColor.V);

    /* All Ys are the same, will fill any alignment padding but that's OK */
    memset(mCurrentFrame, changeExposure(adjustedColor.Y), mFrameHeight * mYStride);
...
void VirtualFakeCameraDevice::drawStripes(void *buffer) {
...
int VirtualFakeCameraDevice::rotateFrame() {
