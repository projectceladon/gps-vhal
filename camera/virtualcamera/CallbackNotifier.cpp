...
// #define LOG_NDEBUG 0
#define LOG_TAG "VirtualCamera_CallbackNotifier"
#include <log/log.h>
...
void CallbackNotifier::setCallbacks(camera_notify_callback notify_cb, camera_data_callback data_cb,
    ...
    ALOGV("%s: %p, %p, %p, %p (%p)", __FUNCTION__, notify_cb, data_cb, data_cb_timestamp,
          get_memory, user);
...
void CallbackNotifier::onNextFrameAvailable(nsecs_t timestamp, VirtualCameraDevice *camera_dev) {
    if (isMessageEnabled(CAMERA_MSG_VIDEO_FRAME) && isVideoRecordingEnabled() &&
        ...
        if (NULL != cam_buff && NULL != cam_buff->data) {
            ...
            camera_dev->getCurrentFrame(cam_buff->data, V4L2_PIX_FMT_YUV420, &frame_timestamp);
    ...
    if (isMessageEnabled(CAMERA_MSG_PREVIEW_FRAME)) {
        ...
        if (NULL != cam_buff && NULL != cam_buff->data) {
            int64_t frame_timestamp = 0L;
            camera_dev->getCurrentFrame(cam_buff->data, camera_dev->getOriginalPixelFormat(),
                                        &frame_timestamp);
    ...
    if (mTakingPicture) {
        ...
        if (isMessageEnabled(CAMERA_MSG_COMPRESSED_IMAGE)) {
            ...
            VirtualCameraDevice::FrameLock lock(*camera_dev);
            ...
            if (thumbWidth > 0 && thumbHeight > 0) {
                if (!createThumbnail(static_cast<const unsigned char *>(frame), width, height,
                                     thumbWidth, thumbHeight, mJpegQuality, exifData)) {
                    // Not really a fatal error, we'll just keep going
                    ALOGE("%s: Failed to create thumbnail for image", __FUNCTION__);
                }
            }
            ...
            status_t res =
                compressor.compressRawImage(frame, width, height, mJpegQuality, exifData);
            if (res == NO_ERROR) {
