...
status_t VirtualFakeCamera3::getCameraCapabilities() {
  const char *key = mFacingBack ? "remote.sf.back_camera_caps"
                                : "remote.sf.front_camera_caps";
...
status_t VirtualFakeCamera3::constructStaticInfo() {
  ...
  size_t rawSizeCount =
      sizeof(kAvailableRawSizes) / sizeof(kAvailableRawSizes[0]);
...
#define ADD_STATIC_ENTRY(name, varptr, count) \
  ...
  if (hasCapability(BACKWARD_COMPATIBLE)) {
    static const int32_t jpegThumbnailSizes[] = {0,   0,   160, 120,
                                                 320, 180, 320, 240};
  ...
  const uint8_t awbLockAvailable =
      hasCapability(BACKWARD_COMPATIBLE)
  ...
  if (hasCapability(DEPTH_OUTPUT)) {
    ...
    static const uint8_t depthIsExclusive =
    ...
    if (buf->stream->format == HAL_PIXEL_FORMAT_BLOB &&
      ...
      if (goodBuffer) {
        ...
        res = mParent->mJpegCompressor->start(
            mCurrentRequest.sensorBuffers, this, &(mCurrentRequest.settings));
        goodBuffer = (res == OK);
      }
      if (goodBuffer) {
        needJpeg = true;
