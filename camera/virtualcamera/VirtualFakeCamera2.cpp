...
/*
 * Contains implementation of a class VirtualFakeCamera2 that encapsulates
...
const uint32_t VirtualFakeCamera2::kAvailableFormats[4] = {
    HAL_PIXEL_FORMAT_RAW16, HAL_PIXEL_FORMAT_BLOB, HAL_PIXEL_FORMAT_RGBA_8888,
    //        HAL_PIXEL_FORMAT_YV12,
    HAL_PIXEL_FORMAT_YCrCb_420_SP};

const uint32_t VirtualFakeCamera2::kAvailableRawSizes[2] = {
    640, 480
    //    mSensorWidth, mSensorHeight
};

const uint64_t VirtualFakeCamera2::kAvailableRawMinDurations[1] = {
    static_cast<uint64_t>(Sensor::kFrameDurationRange[0])};

const uint32_t VirtualFakeCamera2::kAvailableProcessedSizesBack[4] = {
    640, 480, 320, 240
    //    mSensorWidth, mSensorHeight
};

const uint32_t VirtualFakeCamera2::kAvailableProcessedSizesFront[4] = {
    320, 240, 160, 120
    //    mSensorWidth, mSensorHeight
};

const uint64_t VirtualFakeCamera2::kAvailableProcessedMinDurations[1] = {
    static_cast<uint64_t>(Sensor::kFrameDurationRange[0])};

const uint32_t VirtualFakeCamera2::kAvailableJpegSizesBack[2] = {
    640, 480
    //    mSensorWidth, mSensorHeight
};

const uint32_t VirtualFakeCamera2::kAvailableJpegSizesFront[2] = {
    320, 240
    //    mSensorWidth, mSensorHeight
};

const uint64_t VirtualFakeCamera2::kAvailableJpegMinDurations[1] = {
    static_cast<uint64_t>(Sensor::kFrameDurationRange[0])};
...
status_t VirtualFakeCamera2::Initialize(const char *device_name, const char *frame_dims,
    ...
    if (res != OK) {
        ALOGE("%s: Unable to fill in static info: %s (%d)", __FUNCTION__, strerror(-res), res);
        return res;
    }
...
status_t VirtualFakeCamera2::connectCamera(hw_device_t **device) {
    ...
    res = mConfigureThread->run("VirtualFakeCamera2::configureThread");
    if (res != NO_ERROR) return res;

    res = mReadoutThread->run("VirtualFakeCamera2::readoutThread");
    if (res != NO_ERROR) return res;

    res = mControlThread->run("VirtualFakeCamera2::controlThread");
    if (res != NO_ERROR) return res;

    status_t ret = VirtualCamera2::connectCamera(device);
...
status_t VirtualFakeCamera2::plugCamera() {
    {
        Mutex::Autolock l(mMutex);
...
status_t VirtualFakeCamera2::unplugCamera() {
    {
        Mutex::Autolock l(mMutex);
...
camera_device_status_t VirtualFakeCamera2::getHotplugStatus() {
    Mutex::Autolock l(mMutex);
    return mStatusPresent ? CAMERA_DEVICE_STATUS_PRESENT : CAMERA_DEVICE_STATUS_NOT_PRESENT;
}

status_t VirtualFakeCamera2::closeCamera() {
    {
        Mutex::Autolock l(mMutex);
...
status_t VirtualFakeCamera2::getCameraInfo(struct camera_info *info) {
    info->facing = mFacingBack ? CAMERA_FACING_BACK : CAMERA_FACING_FRONT;
    info->orientation = gVirtualCameraFactory.getFakeCameraOrientation();
    return VirtualCamera2::getCameraInfo(info);
}
...
int VirtualFakeCamera2::requestQueueNotify() {
    ALOGV("Request queue notification received");

    ALOG_ASSERT(mRequestQueueSrc != NULL,
                "%s: Request queue src not set, but received queue notification!", __FUNCTION__);
    ALOG_ASSERT(mFrameQueueDst != NULL,
                "%s: Request queue src not set, but received queue notification!", __FUNCTION__);
    ALOG_ASSERT(mStreams.size() != 0, "%s: No streams allocated, but received queue notification!",
                __FUNCTION__);
    return mConfigureThread->newRequestAvailable();
}

int VirtualFakeCamera2::getInProgressCount() {
    Mutex::Autolock l(mMutex);
...
int VirtualFakeCamera2::constructDefaultRequest(int request_template, camera_metadata_t **request) {
...
int VirtualFakeCamera2::allocateStream(uint32_t width, uint32_t height, int format,
    ...
    switch (format) {
        ...
        case HAL_PIXEL_FORMAT_BLOB:
            availableSizes = mFacingBack ? kAvailableJpegSizesBack : kAvailableJpegSizesFront;
            availableSizeCount = mFacingBack ? sizeof(kAvailableJpegSizesBack) / sizeof(uint32_t)
    ...
    for (; resIdx < availableSizeCount; resIdx++) {
        if (availableSizes[resIdx * 2] == width && availableSizes[resIdx * 2 + 1] == height) break;
    }
    if (resIdx == availableSizeCount) {
        ALOGE("%s: Format 0x%x does not support resolution %d, %d", __FUNCTION__, format, width,
              height);
        return BAD_VALUE;
    }
    ...
    switch (format) {
        case HAL_PIXEL_FORMAT_RAW16:
            if (mRawStreamCount >= kMaxRawStreamCount) {
                ALOGE("%s: Cannot allocate another raw stream (%d already allocated)", __FUNCTION__,
                      mRawStreamCount);
    ...
    *max_buffers = kMaxBufferCount;
    ...
    ALOGV("Stream allocated: %d, %d x %d, 0x%x. U: %x, B: %d", *stream_id, width, height, format,
          *usage, *max_buffers);
...
int VirtualFakeCamera2::registerStreamBuffers(uint32_t stream_id, int num_buffers,
    ...
    ALOGV("%s: Stream %d registering %d buffers", __FUNCTION__, stream_id, num_buffers);
    // Need to find out what the final concrete pixel format for our stream is
    // Assumes that all buffers have the same format.
    if (num_buffers < 1) {
        ALOGE("%s: Stream %d only has %d buffers!", __FUNCTION__, stream_id, num_buffers);
        return BAD_VALUE;
    }
    ...
    ALOGV("%s: Stream %d format set to %x, previously %x", __FUNCTION__, stream_id, finalFormat,
          stream.format);
...
int VirtualFakeCamera2::releaseStream(uint32_t stream_id) {
    Mutex::Autolock l(mMutex);
...
int VirtualFakeCamera2::allocateReprocessStreamFromStream(uint32_t output_stream_id,
    ...
    *stream_id = mNextReprocessStreamId;
    ...
    ALOGV("Reprocess stream allocated: %d: %d, %d, 0x%x. Parent stream: %d", *stream_id,
          newStream.width, newStream.height, newStream.format, output_stream_id);

    mNextReprocessStreamId++;
    return NO_ERROR;
}

int VirtualFakeCamera2::releaseReprocessStream(uint32_t stream_id) {
    Mutex::Autolock l(mMutex);
    ...
    if (isReprocessStreamInUse(stream_id)) {
        ALOGE("%s: Cannot release reprocessing stream %d; in use!", __FUNCTION__, stream_id);
        return BAD_VALUE;
    }
...
int VirtualFakeCamera2::triggerAction(uint32_t trigger_id, int32_t ext1, int32_t ext2) {
    Mutex::Autolock l(mMutex);
    ...
    if (trigger_id == CAMERA2_EXT_TRIGGER_TESTING_DISCONNECT) {
        ...
        gVirtualCameraFactory.onStatusChanged(mCameraID, CAMERA_DEVICE_STATUS_NOT_PRESENT);
    }
...
int VirtualFakeCamera2::dump(int fd) {
    String8 result;

    result.appendFormat("    Camera HAL device: VirtualFakeCamera2\n");
...
void VirtualFakeCamera2::signalError() {
...
status_t VirtualFakeCamera2::ConfigureThread::readyToRun() {
...
status_t VirtualFakeCamera2::ConfigureThread::waitUntilRunning() {
    Mutex::Autolock lock(mInputMutex);
    if (!mRunning) {
...
status_t VirtualFakeCamera2::ConfigureThread::newRequestAvailable() {
    waitUntilRunning();
...
bool VirtualFakeCamera2::ConfigureThread::isStreamInUse(uint32_t id) {
...
int VirtualFakeCamera2::ConfigureThread::getInProgressCount() {
    Mutex::Autolock lock(mInputMutex);
    return mRequestCount;
}

bool VirtualFakeCamera2::ConfigureThread::threadLoop() {
    status_t res;
    ...
    if (mRequest == NULL) {
        ...
        res = mParent->mRequestQueueSrc->dequeue_request(mParent->mRequestQueueSrc, &mRequest);
        if (res != NO_ERROR) {
...
bool VirtualFakeCamera2::ConfigureThread::setupCapture() {
    status_t res;
    ...
    res = find_camera_metadata_entry(mRequest, ANDROID_REQUEST_OUTPUT_STREAMS, &streams);
    if (res != NO_ERROR) {
    ...
    if (res != NO_ERROR) {
        ALOGE("%s: error reading frame count tag: %s (%d)", __FUNCTION__, strerror(-res), res);
        mParent->signalError();
        return false;
    }
    ...
    if (res != NO_ERROR) {
        ALOGE("%s: error reading exposure time tag: %s (%d)", __FUNCTION__, strerror(-res), res);
        mParent->signalError();
        return false;
    }
...
bool VirtualFakeCamera2::ConfigureThread::configureNextCapture() {
...
bool VirtualFakeCamera2::ConfigureThread::setupReprocess() {
    status_t res;
    ...
    res = find_camera_metadata_entry(mRequest, ANDROID_REQUEST_INPUT_STREAMS, &reprocessStreams);
    if (res != NO_ERROR) {
        ALOGE("%s: error reading output stream tag", __FUNCTION__);
        mParent->signalError();
        return false;
    ...
    for (size_t i = 0; i < streams.count; i++) {
        ...
        if (s.format != HAL_PIXEL_FORMAT_BLOB) {
            ...
            ALOGE("%s: Non-JPEG output stream %d for reprocess not supported", __FUNCTION__,
                  streamId);
            mParent->signalError();
            return false;
        }
...
bool VirtualFakeCamera2::ConfigureThread::configureNextReprocess() {
    Mutex::Autolock il(mInternalsMutex);
...
bool VirtualFakeCamera2::ConfigureThread::getBuffers() {
    status_t res;
    ...
    for (size_t i = 0; i < mNextBuffers->size(); i++) {
        ...
        if (b.streamId > 0) {
            ...
            if (res != NO_ERROR || b.buffer == NULL) {
                ALOGE("%s: Unable to dequeue buffer from stream %d: %s (%d)", __FUNCTION__,
                      b.streamId, strerror(-res), res);
                mParent->signalError();
                return false;
            }

            /* Lock the buffer from the perspective of the graphics mapper */
            res = GrallocModule::getInstance().lock(*(b.buffer), GRALLOC_USAGE_HW_CAMERA_WRITE, 0,
                                                    0, s.width, s.height, (void **)&(b.img));

            if (res != NO_ERROR) {
                ALOGE("%s: grbuffer_mapper.lock failure: %s (%d)", __FUNCTION__, strerror(-res),
                      res);
            ...
            ALOGV("Configure: Acquiring buffer from reprocess stream %d", -b.streamId);
            ...
            /* Lock the buffer from the perspective of the graphics mapper */
            res = GrallocModule::getInstance().lock(*(b.buffer), GRALLOC_USAGE_HW_CAMERA_READ, 0, 0,
                                                    s.width, s.height, (void **)&(b.img));
            if (res != NO_ERROR) {
                ALOGE("%s: grbuffer_mapper.lock failure: %s (%d)", __FUNCTION__, strerror(-res),
                      res);
...
VirtualFakeCamera2::ReadoutThread::ReadoutThread(VirtualFakeCamera2 *parent)
    : Thread(false),
      mParent(parent),
      mRunning(false),
      mActive(false),
...
status_t VirtualFakeCamera2::ReadoutThread::readyToRun() {
    Mutex::Autolock lock(mInputMutex);
...
status_t VirtualFakeCamera2::ReadoutThread::waitUntilRunning() {
    Mutex::Autolock lock(mInputMutex);
    if (!mRunning) {
...
bool VirtualFakeCamera2::ReadoutThread::waitForReady(nsecs_t timeout) {
    status_t res;
    Mutex::Autolock lock(mInputMutex);
    while (!readyForNextCapture()) {
        ...
        if (res != OK) {
            ALOGE("%s: Error waiting for ready: %s (%d)", __FUNCTION__, strerror(-res), res);
            return false;
        }
    }
    return true;
...
bool VirtualFakeCamera2::ReadoutThread::readyForNextCapture() {
    return (mInFlightTail + 1) % kInFlightQueueSize != mInFlightHead;
}

void VirtualFakeCamera2::ReadoutThread::setNextOperation(bool isCapture, camera_metadata_t *request,
                                                         Buffers *buffers) {
    Mutex::Autolock lock(mInputMutex);
...
bool VirtualFakeCamera2::ReadoutThread::isStreamInUse(uint32_t id) {
...
int VirtualFakeCamera2::ReadoutThread::getInProgressCount() {
    Mutex::Autolock lock(mInputMutex);

    return mRequestCount;
}

bool VirtualFakeCamera2::ReadoutThread::threadLoop() {
    ...
    // Check if we're currently processing or just waiting
    {
        ...
        if (mRequest == NULL) {
            if (mInFlightHead == mInFlightTail) {
                ...
                ALOGV("Ready to read out request %p, %zu buffers", mRequest, mBuffers->size());
            }
        }
    }
    ...
    if (!mIsCapture) {
        res = find_camera_metadata_entry(mRequest, ANDROID_SENSOR_TIMESTAMP, &entry);
        if (res != NO_ERROR) {
            ALOGE("%s: error reading reprocessing timestamp: %s (%d)", __FUNCTION__, strerror(-res),
                  res);
            mParent->signalError();
            return false;
        }
    ...
    res = find_camera_metadata_entry(mRequest, ANDROID_REQUEST_FRAME_COUNT, &entry);
    if (res != NO_ERROR) {
        ALOGE("%s: error reading frame count tag: %s (%d)", __FUNCTION__, strerror(-res), res);
        mParent->signalError();
        return false;
    ...
    res = find_camera_metadata_entry(mRequest, ANDROID_REQUEST_METADATA_MODE, &entry);
    if (res != NO_ERROR) {
        ALOGE("%s: error reading metadata mode tag: %s (%d)", __FUNCTION__, strerror(-res), res);
        mParent->signalError();
        return false;
    }

    // Got sensor data and request, construct frame and send it out
    ALOGV("Readout: Constructing metadata and frames for request %d", frameNumber);
    ...
    if (*entry.data.u8 == ANDROID_REQUEST_METADATA_MODE_FULL) {
        ...
        res = mParent->mFrameQueueDst->dequeue_frame(mParent->mFrameQueueDst, frame_entries,
                                                     frame_data, &frame);
        ...
        if (mIsCapture) {
            add_camera_metadata_entry(frame, ANDROID_SENSOR_TIMESTAMP, &captureTime, 1);
    ...
    if (res != NO_ERROR) {
        ALOGE("%s: Unable to return request buffer to queue: %d", __FUNCTION__, res);
        mParent->signalError();
        return false;
    }
    mRequest = NULL;
    ...
    for (size_t i = 0; i < mBuffers->size(); i++) {
        ...
        ALOGV("Readout:    Buffer %zu: Stream %d, %d x %d, format 0x%x, stride %d", i, b.streamId,
              b.width, b.height, b.format, b.stride);
        if (b.streamId > 0) {
            if (b.format == HAL_PIXEL_FORMAT_BLOB) {
                // Assumes only one BLOB buffer type per capture
                compressedBufferIndex = i;
            } else {
                ALOGV("Readout:    Sending image buffer %zu (%p) to output stream %d", i,
                      (void *)*(b.buffer), b.streamId);
                ...
                if (res != OK) {
                    ALOGE("Error enqueuing image buffer %p: %s (%d)", b.buffer, strerror(-res),
                          res);
                    mParent->signalError();
                }
            }
    ...
    if (compressedBufferIndex == -1) {
        ...
        ALOGV("Readout:  Starting JPEG compression for buffer %d, stream %d", compressedBufferIndex,
...
void VirtualFakeCamera2::ReadoutThread::onJpegDone(const StreamBuffer &jpegBuffer, bool success) {
    status_t res;
    if (!success) {
        ALOGE("%s: Error queueing compressed image buffer %p", __FUNCTION__, jpegBuffer.buffer);
        mParent->signalError();
        return;
    }

    // Write to JPEG output stream
    ALOGV("%s: Compression complete, pushing to stream %d", __FUNCTION__, jpegBuffer.streamId);
...
void VirtualFakeCamera2::ReadoutThread::onJpegInputDone(const StreamBuffer &inputBuffer) {
    status_t res;
    GrallocModule::getInstance().unlock(*(inputBuffer.buffer));
    const ReprocessStream &s = mParent->getReprocessStreamInfo(-inputBuffer.streamId);
    res = s.ops->release_buffer(s.ops, inputBuffer.buffer);
    if (res != OK) {
        ALOGE("Error releasing reprocess buffer %p: %s (%d)", inputBuffer.buffer, strerror(-res),
              res);
        mParent->signalError();
    }
}

status_t VirtualFakeCamera2::ReadoutThread::collectStatisticsMetadata(camera_metadata_t *frame) {
    // Completely fake face rectangles, don't correspond to real faces in scene
    ALOGV("Readout:    Collecting statistics metadata");

    status_t res;
    camera_metadata_entry_t entry;
    res = find_camera_metadata_entry(frame, ANDROID_STATISTICS_FACE_DETECT_MODE, &entry);
    if (res != OK) {
    ...
    int32_t rects[numFaces * 4] = {static_cast<int32_t>(mParent->mSensorWidth * 10 / 20),
    ...
    res = add_camera_metadata_entry(frame, ANDROID_STATISTICS_FACE_RECTANGLES, rects, numFaces * 4);
    if (res != OK) {
        ALOGE("%s: Unable to add face rectangles!", __FUNCTION__);
        return BAD_VALUE;
    }

    res = add_camera_metadata_entry(frame, ANDROID_STATISTICS_FACE_SCORES, scores, numFaces);
    if (res != OK) {
    ...
    res =
        add_camera_metadata_entry(frame, ANDROID_STATISTICS_FACE_LANDMARKS, features, numFaces * 6);
    if (res != OK) {
        ALOGE("%s: Unable to add face landmarks!", __FUNCTION__);
        return BAD_VALUE;
    }

    res = add_camera_metadata_entry(frame, ANDROID_STATISTICS_FACE_IDS, ids, numFaces);
    if (res != OK) {
        ALOGE("%s: Unable to add face scores!", __FUNCTION__);
        return BAD_VALUE;
    }
...
status_t VirtualFakeCamera2::ControlThread::readyToRun() {
    Mutex::Autolock lock(mInputMutex);
...
status_t VirtualFakeCamera2::ControlThread::waitUntilRunning() {
    Mutex::Autolock lock(mInputMutex);
    if (!mRunning) {
...
status_t VirtualFakeCamera2::ControlThread::processRequest(camera_metadata_t *request) {
    Mutex::Autolock lock(mInputMutex);
...
#define READ_IF_OK(res, what, def) (((res) == OK) ? (what) : (uint8_t)(def))
    ...
    mEffectMode = READ_IF_OK(res, mode.data.u8[0], ANDROID_CONTROL_EFFECT_MODE_OFF);

    res = find_camera_metadata_entry(request, ANDROID_CONTROL_SCENE_MODE, &mode);
    mSceneMode = READ_IF_OK(res, mode.data.u8[0], ANDROID_CONTROL_SCENE_MODE_DISABLED);
    ...
    uint8_t aeLockVal = READ_IF_OK(res, mode.data.u8[0], ANDROID_CONTROL_AE_LOCK_ON);
    ...
    if (mAeMode != ANDROID_CONTROL_AE_MODE_OFF) {
        ...
        res = find_camera_metadata_entry(request, ANDROID_SENSOR_EXPOSURE_TIME, &exposureTime);
        if (res == OK) {
...
status_t VirtualFakeCamera2::ControlThread::triggerAction(uint32_t msgType, int32_t ext1,
                                                          int32_t ext2) {
    ...
    switch (msgType) {
        ...
        case CAMERA2_TRIGGER_PRECAPTURE_METERING:
            ...
            break;
        default:
            ALOGE("%s: Unknown action triggered: %d (arguments %d %d)", __FUNCTION__, msgType, ext1,
                  ext2);
            return BAD_VALUE;
    }
    return OK;
...
const nsecs_t VirtualFakeCamera2::ControlThread::kControlCycleDelay = 100 * MSEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMinAfDuration = 500 * MSEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMaxAfDuration = 900 * MSEC;
const float VirtualFakeCamera2::ControlThread::kAfSuccessRate = 0.9;
// Once every 5 seconds
const float VirtualFakeCamera2::ControlThread::kContinuousAfStartRate =
    kControlCycleDelay / 5.0 * SEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMinAeDuration = 500 * MSEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMaxAeDuration = 2 * SEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMinPrecaptureAeDuration = 100 * MSEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMaxPrecaptureAeDuration = 400 * MSEC;
// Once every 3 seconds
const float VirtualFakeCamera2::ControlThread::kAeScanStartRate = kControlCycleDelay / 3000000000.0;

const nsecs_t VirtualFakeCamera2::ControlThread::kNormalExposureTime = 10 * MSEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kExposureJump = 2 * MSEC;
const nsecs_t VirtualFakeCamera2::ControlThread::kMinExposureTime = 1 * MSEC;

bool VirtualFakeCamera2::ControlThread::threadLoop() {
...
int VirtualFakeCamera2::ControlThread::processAfTrigger(uint8_t afMode, uint8_t afState) {
    switch (afMode) {
        ...
        case ANDROID_CONTROL_AF_MODE_CONTINUOUS_PICTURE:
            switch (afState) {
                ...
                case ANDROID_CONTROL_AF_STATE_NOT_FOCUSED_LOCKED:
                    ...
                    break;
                default:
                    ALOGE("Unexpected AF state in CONTINUOUS_PICTURE AF mode: %d", afState);
            }
            break;
...
int VirtualFakeCamera2::ControlThread::maybeStartAfScan(uint8_t afMode, uint8_t afState) {
    if ((afMode == ANDROID_CONTROL_AF_MODE_CONTINUOUS_VIDEO ||
        ...
        if (startScan) {
            ...
            mAfScanDuration =
                ((double)rand() / RAND_MAX) * (kMaxAfDuration - kMinAfDuration) + kMinAfDuration;
...
int VirtualFakeCamera2::ControlThread::updateAfScan(uint8_t afMode, uint8_t afState,
                                                    nsecs_t *maxSleep) {
...
void VirtualFakeCamera2::ControlThread::updateAfState(uint8_t newState, int32_t triggerId) {
    Mutex::Autolock lock(mInputMutex);
    if (mAfState != newState) {
        ALOGV("%s: Autofocus state now %d, id %d", __FUNCTION__, newState, triggerId);
        mAfState = newState;
        mParent->sendNotification(CAMERA2_MSG_AUTOFOCUS, newState, triggerId, 0);
    }
}

int VirtualFakeCamera2::ControlThread::processPrecaptureTrigger(uint8_t aeMode, uint8_t aeState) {
    switch (aeMode) {
        ...
        case ANDROID_CONTROL_AE_MODE_ON_AUTO_FLASH_REDEYE:
            ...
            ALOGD("%s: AE precapture scan start, duration %" PRId64 " ms", __FUNCTION__,
                  mAeScanDuration / 1000000);
    }
    return aeState;
}

int VirtualFakeCamera2::ControlThread::maybeStartAeScan(uint8_t aeMode, bool aeLocked,
                                                        uint8_t aeState) {
    ...
    switch (aeMode) {
        ...
        case ANDROID_CONTROL_AE_MODE_ON_AUTO_FLASH_REDEYE: {
            ...
            if (startScan) {
                mAeScanDuration = ((double)rand() / RAND_MAX) * (kMaxAeDuration - kMinAeDuration) +
                                  kMinAeDuration;
...
int VirtualFakeCamera2::ControlThread::updateAeScan(uint8_t aeMode, bool aeLock, uint8_t aeState,
                                                    nsecs_t *maxSleep) {
    if (aeLock && aeState != ANDROID_CONTROL_AE_STATE_PRECAPTURE) {
        ...
        if (mAeScanDuration <= 0) {
            ...
            int64_t exposureDelta = ((double)rand() / RAND_MAX) * 2 * kExposureJump - kExposureJump;
            Mutex::Autolock lock(mInputMutex);
...
void VirtualFakeCamera2::ControlThread::updateAeState(uint8_t newState, int32_t triggerId) {
    Mutex::Autolock lock(mInputMutex);
    if (mAeState != newState) {
        ALOGV("%s: Autoexposure state now %d, id %d", __FUNCTION__, newState, triggerId);
        mAeState = newState;
...
status_t VirtualFakeCamera2::constructStaticInfo(camera_metadata_t **info, bool sizeRequest) const {
    size_t entryCount = 0;
    size_t dataCount = 0;
    status_t ret;

#define ADD_OR_SIZE(tag, data, count)                                                           \
    if ((ret = addOrSize(*info, sizeRequest, &entryCount, &dataCount, tag, data, count)) != OK) \
    return ret
    ...
    ADD_OR_SIZE(ANDROID_LENS_INFO_HYPERFOCAL_DISTANCE, &hyperFocalDistance, 1);
    ...
    ADD_OR_SIZE(ANDROID_LENS_INFO_AVAILABLE_OPTICAL_STABILIZATION, &availableOpticalStabilization,
                1);
    ...
    int32_t lensFacing = mFacingBack ? ANDROID_LENS_FACING_BACK : ANDROID_LENS_FACING_FRONT;
    ADD_OR_SIZE(ANDROID_LENS_FACING, &lensFacing, 1);

    // android.sensor

    ADD_OR_SIZE(ANDROID_SENSOR_INFO_EXPOSURE_TIME_RANGE, Sensor::kExposureTimeRange, 2);

    ADD_OR_SIZE(ANDROID_SENSOR_INFO_MAX_FRAME_DURATION, &Sensor::kFrameDurationRange[1], 1);

    ADD_OR_SIZE(ANDROID_SENSOR_INFO_SENSITIVITY_RANGE, Sensor::kSensitivityRange,
                sizeof(Sensor::kSensitivityRange) / sizeof(int32_t));

    ADD_OR_SIZE(ANDROID_SENSOR_INFO_COLOR_FILTER_ARRANGEMENT, &Sensor::kColorFilterArrangement, 1);
    ...
    static const int32_t blackLevelPattern[4] = {
        static_cast<int32_t>(Sensor::kBlackLevel), static_cast<int32_t>(Sensor::kBlackLevel),
        static_cast<int32_t>(Sensor::kBlackLevel), static_cast<int32_t>(Sensor::kBlackLevel)};
    ...
    ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_RAW_SIZES, kAvailableRawSizes,
                sizeof(kAvailableRawSizes) / sizeof(uint32_t));

    ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_RAW_MIN_DURATIONS, kAvailableRawMinDurations,
                sizeof(kAvailableRawMinDurations) / sizeof(uint64_t));

    if (mFacingBack) {
        ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_PROCESSED_SIZES, kAvailableProcessedSizesBack,
                    sizeof(kAvailableProcessedSizesBack) / sizeof(uint32_t));
    } else {
        ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_PROCESSED_SIZES, kAvailableProcessedSizesFront,
                    sizeof(kAvailableProcessedSizesFront) / sizeof(uint32_t));
    }

    ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_PROCESSED_MIN_DURATIONS, kAvailableProcessedMinDurations,
                sizeof(kAvailableProcessedMinDurations) / sizeof(uint64_t));

    if (mFacingBack) {
        ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_JPEG_SIZES, kAvailableJpegSizesBack,
                    sizeof(kAvailableJpegSizesBack) / sizeof(uint32_t));
    } else {
        ADD_OR_SIZE(ANDROID_SCALER_AVAILABLE_JPEG_SIZES, kAvailableJpegSizesFront,
                    sizeof(kAvailableJpegSizesFront) / sizeof(uint32_t));
    }
    ...
    static const uint8_t availableFaceDetectModes[] = {ANDROID_STATISTICS_FACE_DETECT_MODE_OFF,
                                                       ANDROID_STATISTICS_FACE_DETECT_MODE_SIMPLE,
                                                       ANDROID_STATISTICS_FACE_DETECT_MODE_FULL};

    ADD_OR_SIZE(ANDROID_STATISTICS_INFO_AVAILABLE_FACE_DETECT_MODES, availableFaceDetectModes,
    ...
    static const uint8_t availableSceneModes[] = {ANDROID_CONTROL_SCENE_MODE_DISABLED};
    ...
    ADD_OR_SIZE(ANDROID_CONTROL_AVAILABLE_EFFECTS, availableEffects, sizeof(availableEffects));
    ...
    ADD_OR_SIZE(ANDROID_CONTROL_AE_AVAILABLE_MODES, availableAeModes, sizeof(availableAeModes));

    static const camera_metadata_rational exposureCompensationStep = {1, 3};
    ADD_OR_SIZE(ANDROID_CONTROL_AE_COMPENSATION_STEP, &exposureCompensationStep, 1);
    ...
    ADD_OR_SIZE(ANDROID_CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES, availableTargetFpsRanges,
                sizeof(availableTargetFpsRanges) / sizeof(int32_t));

    static const uint8_t availableAntibandingModes[] = {ANDROID_CONTROL_AE_ANTIBANDING_MODE_OFF,
                                                        ANDROID_CONTROL_AE_ANTIBANDING_MODE_AUTO};
    ADD_OR_SIZE(ANDROID_CONTROL_AE_AVAILABLE_ANTIBANDING_MODES, availableAntibandingModes,
    ...
    ADD_OR_SIZE(ANDROID_CONTROL_AWB_AVAILABLE_MODES, availableAwbModes, sizeof(availableAwbModes));
    ...
    ADD_OR_SIZE(ANDROID_CONTROL_AVAILABLE_VIDEO_STABILIZATION_MODES, availableVstabModes,
...
status_t VirtualFakeCamera2::constructDefaultRequest(int request_template,
...
#define ADD_OR_SIZE(tag, data, count)                                                              \
    if ((ret = addOrSize(*request, sizeRequest, &entryCount, &dataCount, tag, data, count)) != OK) \
    return ret
    ...
    /** android.color */
    static const float colorTransform[9] = {1.0f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 1.f};
    ...
    /** android.scaler */
    static const int32_t cropRegion[3] = {0, 0, static_cast<int32_t>(mSensorWidth)};
    ...
    static const int32_t controlRegions[5] = {0, 0, static_cast<int32_t>(mSensorWidth),
...
status_t VirtualFakeCamera2::addOrSize(camera_metadata_t *request, bool sizeRequest,
                                       size_t *entryCount, size_t *dataCount, uint32_t tag,
                                       const void *entryData, size_t entryDataCount) {
    if (!sizeRequest) {
...
bool VirtualFakeCamera2::isStreamInUse(uint32_t id) {
    ...
    if (mConfigureThread->isStreamInUse(id) || mReadoutThread->isStreamInUse(id) ||
...
bool VirtualFakeCamera2::isReprocessStreamInUse(uint32_t id) {
    // TODO: implement
    return false;
}

const Stream &VirtualFakeCamera2::getStreamInfo(uint32_t streamId) {
    Mutex::Autolock lock(mMutex);

    return mStreams.valueFor(streamId);
}

const ReprocessStream &VirtualFakeCamera2::getReprocessStreamInfo(uint32_t streamId) {
    Mutex::Autolock lock(mMutex);
