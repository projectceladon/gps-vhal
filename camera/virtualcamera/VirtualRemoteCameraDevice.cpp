...
VirtualRemoteCameraDevice::VirtualRemoteCameraDevice(VirtualRemoteCamera *camera_hal)
    : VirtualCameraDevice(camera_hal), mRemoteClient() {}
...
status_t VirtualRemoteCameraDevice::Initialize(const char *device_name) {
    ...
    if (res == NO_ERROR) {
        ALOGV("%s: Connected to the virtual camera service '%s'", __FUNCTION__, device_name);
...
status_t VirtualRemoteCameraDevice::connectDevice() {
    ...
    if (isConnected()) {
        ALOGW("%s: Remote camera device '%s' is already connected.", __FUNCTION__,
...
status_t VirtualRemoteCameraDevice::disconnectDevice() {
    ...
    Mutex::Autolock locker(&mObjectLock);
    if (!isConnected()) {
        ALOGW("%s: Remote camera device '%s' is already disconnected.", __FUNCTION__,
              (const char *)mDeviceName);
        return NO_ERROR;
    }
    if (isStarted()) {
    ...
    if (res == NO_ERROR) {
        ALOGV("%s: Disonnected from device '%s'", __FUNCTION__, (const char *)mDeviceName);
        mState = ECDS_INITIALIZED;
    } else {
        ALOGE("%s: Disconnection from device '%s' failed", __FUNCTION__, (const char *)mDeviceName);
    }

    return res;
}

status_t VirtualRemoteCameraDevice::startDevice(int width, int height, uint32_t pix_fmt) {
    ALOGV("%s", __FUNCTION__);

    Mutex::Autolock locker(&mObjectLock);
    if (!isConnected()) {
        ALOGE("%s: Remote camera device '%s' is not connected.", __FUNCTION__,
              (const char *)mDeviceName);
        return EINVAL;
    }
    if (isStarted()) {
        ALOGW("%s: Remote camera device '%s' is already started.", __FUNCTION__,
              (const char *)mDeviceName);
        return NO_ERROR;
    }
    ...
    if (res == NO_ERROR) {
        ALOGV("%s: Remote camera device '%s' is started for %.4s[%dx%d] frames", __FUNCTION__,
              (const char *)mDeviceName, reinterpret_cast<const char *>(&mPixelFormat), mFrameWidth,
              mFrameHeight);
        mState = ECDS_STARTED;
    } else {
        ALOGE("%s: Unable to start device '%s' for %.4s[%dx%d] frames", __FUNCTION__,
              (const char *)mDeviceName, reinterpret_cast<const char *>(&pix_fmt), width, height);
    }
...
status_t VirtualRemoteCameraDevice::stopDevice() {
    ...
    Mutex::Autolock locker(&mObjectLock);
    if (!isStarted()) {
        ALOGW("%s: Remote camera device '%s' is not started.", __FUNCTION__,
              (const char *)mDeviceName);
        return NO_ERROR;
    }
...
status_t VirtualRemoteCameraDevice::getCurrentFrame(void *buffer, uint32_t pixelFormat,
                                                    int64_t *timestamp) {
    ALOGV("%s", __FUNCTION__);
...
status_t VirtualRemoteCameraDevice::getCurrentPreviewFrame(void *buffer, int64_t *timestamp) {
    ALOGV("%s", __FUNCTION__);
...
bool VirtualRemoteCameraDevice::produceFrame(void *buffer, int64_t *timestamp) {
    ...
    status_t query_res = mRemoteClient.queryFrame(
        rawFrame, previewFrame, mFrameBufferSize, mTotalPixels * 4, mWhiteBalanceScale[0],
        mWhiteBalanceScale[1], mWhiteBalanceScale[2], mExposureCompensation, timestamp);
    if (query_res != NO_ERROR) {
        ALOGE("%s: Unable to get current video frame: %s", __FUNCTION__, strerror(query_res));
        return false;
    }
    return true;
}

void *VirtualRemoteCameraDevice::getPrimaryBuffer() { return &mFrameBufferPairs[0]; }
void *VirtualRemoteCameraDevice::getSecondaryBuffer() { return &mFrameBufferPairs[1]; }
