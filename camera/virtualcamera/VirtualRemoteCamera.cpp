...
/*
 * Contains implementation of a class VirtualRemoteCamera that encapsulates
...
VirtualRemoteCamera::VirtualRemoteCamera(int cameraId, struct hw_module_t *module)
    : VirtualCamera(cameraId, module), mRemoteCameraDevice(this) {}

VirtualRemoteCamera::~VirtualRemoteCamera() {}

/****************************************************************************
 * VirtualCamera virtual overrides.
 ***************************************************************************/

status_t VirtualRemoteCamera::Initialize(const char *device_name, const char *frame_dims,
                                         const char *facing_dir) {
    ALOGV("%s:\n   Name=%s\n   Facing '%s'\n   Dimensions=%s", __FUNCTION__, device_name,
          facing_dir, frame_dims);
    /* Save dimensions. */
    mFrameDims = frame_dims;

    /* Initialize camera device. */
    status_t res = mRemoteCameraDevice.Initialize(device_name);
    if (res != NO_ERROR) {
        return res;
    }
    ...
    mParameters.set(VirtualCamera::FACING_KEY, facing_dir);
    mParameters.set(VirtualCamera::ORIENTATION_KEY,
                    gVirtualCameraFactory.getRemoteCameraOrientation());
    mParameters.set(CameraParameters::KEY_ROTATION,
                    gVirtualCameraFactory.getRemoteCameraOrientation());
    mParameters.set(CameraParameters::KEY_SUPPORTED_PICTURE_SIZES, sizes.c_str());
    mParameters.set(CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES, sizes.c_str());
    mParameters.set(CameraParameters::KEY_SUPPORTED_VIDEO_SIZES, sizes.c_str());

    std::string preferred_size =
        std::to_string(resolutions[0].first) + "x" + std::to_string(resolutions[0].second);
    mParameters.set(CameraParameters::KEY_PREFERRED_PREVIEW_SIZE_FOR_VIDEO, preferred_size.c_str());
    ...
    ALOGV("%s: Remote camera %s is initialized. Current frame is %dx%d", __FUNCTION__, device_name,
          x, y);

    return NO_ERROR;
}

VirtualCameraDevice *VirtualRemoteCamera::getCameraDevice() { return &mRemoteCameraDevice; }
