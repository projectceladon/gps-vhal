...
namespace android
{
                                             ...
                                             const char *facing_dir)
    {
        ...
        * dimension exceeding that of the max area resolution. */
        ...
        std::string preferred_size = std::to_string(resolutions[0].first) + "x" + std::to_string(resolutions[0].second);
        ...
        */
        ...
        ALOGV("%s: Remote camera %s is initialized. Current frame is %dx%d",
              __FUNCTION__, device_name, x, y);

        return NO_ERROR;
    }

    VirtualCameraDevice *VirtualRemoteCamera::getCameraDevice()
    {
        return &mRemoteCameraDevice;
    }
