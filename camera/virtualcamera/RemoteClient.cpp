...
RemoteQuery::~RemoteQuery() { RemoteQuery::resetQuery(); }
...
status_t RemoteQuery::createQuery(const char *name, const char *param) {
    ...
    if (required > sizeof(mQueryPrealloc)) {
        ...
        if (mQuery == NULL) {
            ALOGE("%s: Unable to allocate %zu bytes for query buffer", __FUNCTION__, required);
...
RemoteClient::RemoteClient() : mPipeFD(-1), mCameraSocketFD(-1) {
...
RemoteClient::~RemoteClient() {
    ...
    if (mCameraSocketFD >= 0) {
        mCameraSocketFD = -1;
        ALOGV(
            "%s Set current camera socket to -1 without really close as it "
...
status_t RemoteClient::connectClient(const char *param) {
    ...
    if (mCameraSocketFD < 0) {
        ALOGE("%s: Camera client is not connected. mCameraSocketFD = %d", __FUNCTION__,
              mCameraSocketFD);
        return EINVAL;
    }
...
void RemoteClient::disconnectClient() {
    ...
    if (mCameraSocketFD >= 0) {
        ALOGV(
            "%s Set camera client from %d to -1 than close as the socket "
...
status_t RemoteClient::sendMessage(const void *data, size_t data_size) {
    ...
    if (written == data_size) {
        ...
        ALOGE("%s: Error sending data via socket (%d): '%s'", __FUNCTION__, mCameraSocketFD,
              strerror(errno));
...
status_t RemoteClient::receiveMessage(void **data, size_t *data_size) {
    ...
    *data = malloc(payload_size);
    if (*data == NULL) {
        ALOGE("%s: Unable to allocate %zu bytes payload buffer", __FUNCTION__, payload_size);
        return ENOMEM;
    }
    ...
    if (static_cast<size_t>(rd_res) == payload_size) {
        *data_size = payload_size;
        ...
        ALOGE("%s: Read size %d doesnt match expected payload size %zu: %s", __FUNCTION__, rd_res,
              payload_size, strerror(errno));
        free(*data);
        *data = NULL;
        return errno ? errno : EIO;
    }
...
status_t RemoteClient::doQuery(RemoteQuery *query) {
    ...
    if (res != NO_ERROR) {
        ALOGE("%s: Send the header of query '%s' failed: %s", __FUNCTION__, query->mQuery,
              strerror(res));
        return res;
    }
    if (query->csi.data_size > 0) {
        ...
        if (res != NO_ERROR) {
            ALOGE("%s: Send query '%s' failed: %s", __FUNCTION__, query->mQuery, strerror(res));
            return res;
        }
    }

    /* Read the response. */
    res = receiveMessage(reinterpret_cast<void **>(&query->mReplyBuffer), &query->mReplySize);
    if (res == NO_ERROR) {
        LOGQ("Response to query '%s': Status = '%.2s', %d bytes in response", query->mQuery,
             query->mReplyBuffer, query->mReplySize);
    } else {
        ALOGE("%s Response to query '%s' has failed: %s", __FUNCTION__, query->mQuery,
              strerror(res));
    }

    /* Complete the query, and return its completion handling status. */
    const status_t res1 = query->completeQuery(res);
    ALOGE_IF(res1 != NO_ERROR && res1 != res, "%s: Error %d in query '%s' completion", __FUNCTION__,
             res1, query->mQuery);
    return res1;
}
...
FactoryRemoteClient::FactoryRemoteClient() : RemoteClient() {}
...
status_t FactoryRemoteClient::listCameras(char **list) {
    ...
    *list = (char *)malloc(query.mReplyDataSize);
    if (*list != NULL) {
        ...
        ALOGE("%s: Unable to allocate %zu bytes", __FUNCTION__, query.mReplyDataSize);
        return ENOMEM;
    }
}
...
CameraRemoteClient::CameraRemoteClient() : RemoteClient() {}
...
status_t CameraRemoteClient::queryStart(uint32_t pixel_format, int width, int height) {
    ALOGV("%s", __FUNCTION__);

    char query_str[256];
    snprintf(query_str, sizeof(query_str), "%s dim=%dx%d pix=%d", mQueryStart, width, height,
             pixel_format);
    RemoteQuery query(query_str);
...
status_t CameraRemoteClient::queryFrame(void *vframe, void *pframe, size_t vframe_size,
                                        size_t pframe_size, float r_scale, float g_scale,
                                        float b_scale, float exposure_comp, int64_t *frame_time) {
    ALOGV("%s", __FUNCTION__);

    char query_str[256];
    memset(query_str, 0, sizeof(query_str));
    snprintf(query_str, sizeof(query_str),
             "%s video=%zu preview=%zu whiteb=%g,%g,%g expcomp=%g time=%d", mQueryFrame,
             (vframe && vframe_size) ? vframe_size : 0, (pframe && pframe_size) ? pframe_size : 0,
             r_scale, g_scale, b_scale, exposure_comp, frame_time != nullptr ? 1 : 0);
    RemoteQuery query(query_str);
    ...
    if (vframe != NULL && vframe_size != 0) {
        ...
        if ((query.mReplyDataSize - cur_offset) >= vframe_size) {
            ...
            ALOGE("%s: Reply %zu bytes is to small to contain %zu bytes video frame", __FUNCTION__,
                  query.mReplyDataSize - cur_offset, vframe_size);
            return EINVAL;
        }
    }
