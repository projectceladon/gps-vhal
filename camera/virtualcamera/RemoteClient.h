...
#ifndef HW_EMULATOR_CAMERA_REMOTE_CLIENT_H
#define HW_EMULATOR_CAMERA_REMOTE_CLIENT_H

/*
 * Contains declaration of classes that encapsulate connection to camera services
 * in the emulator via remote pipe.
 */
 ...
 * Guest exchanges data with the emulator via queries sent over the remote pipe.
...
class RemoteQuery {
public:
    /* Constructs an uninitialized RemoteQuery instance. */
    RemoteQuery();

    /* Constructs and initializes RemoteQuery instance for a query.
     * Param:
     ...
     */
    explicit RemoteQuery(const char *query_string);

    /* Constructs and initializes RemoteQuery instance for a query with parameters.
     * Param:
     *  query_name - Query name.
     *  query_param - Query parameters. Can be NULL.
     */
    RemoteQuery(const char *query_name, const char *query_param);

    /* Destructs RemoteQuery instance. */
    ~RemoteQuery();
...
/* Encapsulates a connection to the 'camera' service in the emulator via remote
 * pipe.
 */
class RemoteClient {
public:
    /* Constructs RemoteClient instance. */
    RemoteClient();

    /* Destructs RemoteClient instance. */
...
public:
    /* Connects to the 'camera' service in the emulator via remote pipe.
     * Param:
     ...
     *      connected to the host. Another one is an 'virtual camera' type of
     ...
     *      connection to an 'virtual camera' service, where camera is identified
     ...
     *      with an 'virtual camera' service. Parameters defining the virtual
    ...
    ssize_t read_spec_size(int fd, void *buf, size_t count);
    ssize_t write_spec_size(int fd, const void *buf, size_t count);
     ...
     */
    virtual status_t doQuery(RemoteQuery *query);
...
/* Encapsulates RemoteClient for the 'factory' service. */
class FactoryRemoteClient : public RemoteClient {
public:
    /* Constructs FactoryRemoteClient instance. */
    FactoryRemoteClient();

    /* Destructs FactoryRemoteClient instance. */
    ~FactoryRemoteClient();
...
public:
     ...
     *        name must be used for subsequent connection to the 'virtual camera'
    ...
    /****************************************************************************
     * Names of the queries available for the virtual camera factory.
     ***************************************************************************/
...
/* Encapsulates RemoteClient for an 'virtual camera' service.
 */
class CameraRemoteClient : public RemoteClient {
public:
    /* Constructs CameraRemoteClient instance. */
    CameraRemoteClient();

    /* Destructs CameraRemoteClient instance. */
    ~CameraRemoteClient();
...
public:
     ...
     *  NO_ERROR on success, or an appropriate error status on failure.
     */
    status_t queryFrame(void *vframe, void *pframe, size_t vframe_size, size_t pframe_size,
                        float r_scale, float g_scale, float b_scale, float exposure_comp,
                        int64_t *frame_time);

    /****************************************************************************
     * Names of the queries available for the virtual camera.
     ***************************************************************************/
...
#endif /* HW_EMULATOR_CAMERA_REMOTE_CLIENT_H */
