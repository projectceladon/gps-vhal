...
/*
 * Contains implementation of an abstract class VirtualCameraDevice that defines
 * functionality expected from an virtual physical camera device:
...
VirtualCameraDevice::VirtualCameraDevice(VirtualCamera *camera_hal)
    : mObjectLock(),
...
VirtualCameraDevice::~VirtualCameraDevice() {
    ALOGV("VirtualCameraDevice destructor");
...
status_t VirtualCameraDevice::Initialize(const char *name) {
    if (isInitialized()) {
        ALOGW("%s: Virtual camera device is already initialized: mState = %d", __FUNCTION__,
              mState);
        return NO_ERROR;
    }
...
status_t VirtualCameraDevice::startDeliveringFrames(bool one_burst) {
...
status_t VirtualCameraDevice::stopDeliveringFrames() {
    ALOGV("%s", __FUNCTION__);
...
status_t VirtualCameraDevice::setPreviewFrameRate(int framesPerSecond) {
...
void VirtualCameraDevice::setExposureCompensation(const float ev) {
    ALOGV("%s", __FUNCTION__);
...
void VirtualCameraDevice::initializeWhiteBalanceModes(const char *mode, const float r_scale,
                                                      const float b_scale) {
...
void VirtualCameraDevice::setWhiteBalanceMode(const char *mode) {
 ...
 */
void VirtualCameraDevice::changeWhiteBalance(uint8_t &y, uint8_t &u, uint8_t &v) const {
...
void VirtualCameraDevice::checkAutoFocusTrigger() {
...
status_t VirtualCameraDevice::getCurrentFrameImpl(const uint8_t *source, uint8_t *dest,
                                                  uint32_t pixelFormat) const {
    if (pixelFormat == mPixelFormat) {
        memcpy(dest, source, mFrameBufferSize);
        return NO_ERROR;
    } else if (pixelFormat == V4L2_PIX_FMT_YUV420 && mPixelFormat == V4L2_PIX_FMT_YVU420) {
        ...
        if (mYStride == mFrameWidth) {
            ...
            for (int y = 0; y < mFrameHeight; ++y) {
                memcpy(dest + y * mFrameWidth, source + y * mYStride, mFrameWidth);
            }
        }
        ...
        if (mUVStride == mFrameWidth / 2) {
            ...
            for (int i = 0; i < 2; ++i) {
                for (int y = 0; y < mFrameHeight / 2; ++y) {
                    memcpy(uvDest + y * (mFrameWidth / 2), uvSource + y * mUVStride,
                           mFrameWidth / 2);
                }
    ...
    ALOGE("%s: Invalid pixel format conversion [%.4s to %.4s] requested", __FUNCTION__,
          reinterpret_cast<const char *>(&mPixelFormat),
          reinterpret_cast<const char *>(&pixelFormat));
    return EINVAL;
}

status_t VirtualCameraDevice::getCurrentFrame(void *buffer, uint32_t pixelFormat,
                                              int64_t *timestamp) {
    if (!isStarted()) {
        ALOGE("%s: Device is not started", __FUNCTION__);
        return EINVAL;
...
status_t VirtualCameraDevice::getCurrentPreviewFrame(void *buffer, int64_t *timestamp) {
    if (!isStarted()) {
        ALOGE("%s: Device is not started", __FUNCTION__);
        return EINVAL;
    }
...
const void *VirtualCameraDevice::getCurrentFrame() {
...
VirtualCameraDevice::FrameLock::FrameLock(VirtualCameraDevice &cameraDevice)
    : mCameraDevice(cameraDevice) {
    mCameraDevice.lockCurrentFrame();
}

VirtualCameraDevice::FrameLock::~FrameLock() { mCameraDevice.unlockCurrentFrame(); }

status_t VirtualCameraDevice::setAutoFocus() {
    mTriggerAutoFocus = true;
    return NO_ERROR;
}

status_t VirtualCameraDevice::cancelAutoFocus() {
    mTriggerAutoFocus = false;
    return NO_ERROR;
}

bool VirtualCameraDevice::requestRestart(int width, int height, uint32_t pixelFormat,
                                         bool takingPicture, bool oneBurst) {
    if (mCameraThread.get() == nullptr) {
        ALOGE("%s: No thread alive to perform the restart, is preview on?", __FUNCTION__);
        return false;
    }
    mCameraThread->requestRestart(width, height, pixelFormat, takingPicture, oneBurst);
    return true;
}

/****************************************************************************
 * Virtual camera device private API
 ***************************************************************************/

status_t VirtualCameraDevice::commonStartDevice(int width, int height, uint32_t pix_fmt) {
    ...
    ALOGV("%s: Allocated %zu bytes for %d pixels in %.4s[%dx%d] frame", __FUNCTION__,
          mFrameBufferSize, mTotalPixels, reinterpret_cast<const char *>(&mPixelFormat),
          mFrameWidth, mFrameHeight);
    return NO_ERROR;
}

void VirtualCameraDevice::commonStopDevice() {
...
status_t VirtualCameraDevice::startWorkerThread(bool one_burst) {
    ALOGV("%s", __FUNCTION__);

    if (!isInitialized()) {
        ALOGE("%s: Virtual camera device is not initialized", __FUNCTION__);
        return EINVAL;
    }
    ...
    if (res != NO_ERROR) {
        ALOGE("%s: Unable to start CameraThread: %s", __FUNCTION__, strerror(res));
        return res;
    }
...
status_t VirtualCameraDevice::stopWorkerThread() {
    ALOGV("%s", __FUNCTION__);
...
VirtualCameraDevice::CameraThread::CameraThread(VirtualCameraDevice *dev, ProduceFrameFunc producer,
                                                void *producerOpaque)
...
const void *VirtualCameraDevice::CameraThread::getPrimaryBuffer() const {
...
int64_t VirtualCameraDevice::CameraThread::getPrimaryTimestamp() const {
    if (mFrameProducer.get()) {
...
void VirtualCameraDevice::CameraThread::lockPrimaryBuffer() { mFrameProducer->lockPrimaryBuffer(); }

void VirtualCameraDevice::CameraThread::unlockPrimaryBuffer() {
    mFrameProducer->unlockPrimaryBuffer();
}

bool VirtualCameraDevice::CameraThread::waitForFrameOrTimeout(nsecs_t timeout) {
...
bool VirtualCameraDevice::CameraThread::inWorkerThread() {
    /* Wait till FPS timeout expires, or thread exit message is received. */
    nsecs_t wakeAt = mCurFrameTimestamp + 1000000000.0 / mCameraDevice->mFramesPerSecond;
...
status_t VirtualCameraDevice::CameraThread::onThreadStart() {
    void *primaryBuffer = mCameraDevice->getPrimaryBuffer();
    void *secondaryBuffer = mCameraDevice->getSecondaryBuffer();
    mFrameProducer = new FrameProducer(mCameraDevice, mProducerFunc, mProducerOpaque, primaryBuffer,
                                       secondaryBuffer);
...
void VirtualCameraDevice::CameraThread::onThreadExit() {
    if (mFrameProducer.get()) {
...
VirtualCameraDevice::CameraThread::FrameProducer::FrameProducer(VirtualCameraDevice *dev,
                                                                ProduceFrameFunc producer,
                                                                void *opaque, void *primaryBuffer,
...
const void *VirtualCameraDevice::CameraThread::FrameProducer::getPrimaryBuffer() const {
    return mPrimaryBuffer;
}

int64_t VirtualCameraDevice::CameraThread::FrameProducer::getPrimaryTimestamp() const {
    return mPrimaryTimestamp;
}

void VirtualCameraDevice::CameraThread::FrameProducer::lockPrimaryBuffer() { mBufferMutex.lock(); }
void VirtualCameraDevice::CameraThread::FrameProducer::unlockPrimaryBuffer() {
    mBufferMutex.unlock();
}

void VirtualCameraDevice::CameraThread::requestRestart(int width, int height, uint32_t pixelFormat,
                                                       bool takingPicture, bool oneBurst) {
...
bool VirtualCameraDevice::CameraThread::FrameProducer::hasFrame() const { return mHasFrame; }

bool VirtualCameraDevice::CameraThread::checkRestartRequest() {
    Mutex::Autolock lock(mRequestMutex);
    if (mRestartRequested) {
        ...
        res = mCameraDevice->startDevice(mRestartWidth, mRestartHeight, mRestartPixelFormat);
        if (res != NO_ERROR) {
...
bool VirtualCameraDevice::CameraThread::FrameProducer::inWorkerThread() {
    nsecs_t nextFrame = mLastFrame + 1000000000 / mCameraDevice->mFramesPerSecond;
    nsecs_t now = systemTime(SYSTEM_TIME_MONOTONIC);
...
void VirtualCameraDevice::lockCurrentFrame() { mCameraThread->lockPrimaryBuffer(); }

void VirtualCameraDevice::unlockCurrentFrame() { mCameraThread->unlockPrimaryBuffer(); }
